    ...
    public static class HttpHelper
    {
        ...
        public static string HttpUploadFile(string url, string[] files, NameValueCollection data, Encoding encoding)
        {
            ...
            using (Stream stream = request.GetRequestStream())
            {
                ...
                {
                    Match M = Regex.Match(files[i], "([^=]+)=(.+)");
                    stream.Write(boundarybytes, 0, boundarybytes.Length);
                    string header = string.Format(headerTemplate, M.Groups[1].Value, Path.GetFileName(M.Groups[2].Value));
                    byte[] headerbytes = encoding.GetBytes(header);
                    stream.Write(headerbytes, 0, headerbytes.Length);
                    using (FileStream fileStream = new FileStream(M.Groups[2].Value, FileMode.Open, FileAccess.Read))
                    {
