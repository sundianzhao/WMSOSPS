(function ($) {
    ...
    $.fn.treeview = function (settings) {
        ...
        function buildnode(nd, ht, deep, path, isend) {
            ...
            if (nd.hasChildren) {
                if (nd.img == -1) {
                    ...
                    if (!!nd.img) {
                        var html = "<i class=\"" + nd.img + "\"></i>";
                        if (nd.img.indexOf('icon') < 0) {
                            html = "<img  src=\"" + nd.img + "\"></i>"
                        }
                        ht.push(html);
                    } else {
        ...
        function noderclick(e) {
            ...
            if (e.which == 3) {
                ...
                $("." + id).find('div').removeClass("bbit-tree-selected");
                $(this).addClass("bbit-tree-selected");
                if (dfop.onnoderclick) {
                    dfop.onnoderclick.call(this, e, item);
                }
            }
        }
        function expandnode() {
        ...
        function bindevent() {
            ...
            }).click(nodeclick).mousedown(noderclick)
    ...
    $.fn.getCheckedNodesValue = function () {
        var $id = $(this);
        var _length = $id.attr('id').trim().length + 1;
        var value = []
        $id.find('.bbit-tree-node-cb').each(function () {
    ...
    $.fn.getCheckedNodesAttr = function () {
        var $id = $(this);
        var _length = $id.attr('id').trim().length + 1;
        var value = []
        $id.find('.bbit-tree-node-cb').each(function () {
            ...
            if (_src == 'checkbox_1.png' || _src == 'checkbox_2.png') {
                var _value = $(this).attr('id').substring(parseInt(_length)).replace(/_/g, "-");
                _value = _value.substring(0, _value.length - 3);
                var _text = $(this).parent().find(".bbit-tree-node-text").text();
                var data = { code: _value, name: _text };
                value.push(data)
            }
        });
        return value;
    ...
    //只获取中国和省份（如果选择的省份中城市没有被全选，此时省份不会被筛选出来）
    $.fn.getCheckedNodesTexts = function () {
        var $id = $(this);
        var value = [];
        //只获取中国和省份（如果选择的省份中城市没有被全选，此时省份不会被筛选出来）
        $id.find('.bbit-tree-node-cb').not($id.find('ul li ul li ul .bbit-tree-node-cb')).each(function () {
            var _src = $(this).attr('src');
            _src = _src.substr(_src.lastIndexOf("/") + 1);
            if (_src == 'checkbox_1.png') {
                var _text = $(this).parent().find(".bbit-tree-node-text").text();
        ...
        //获取那些没有全部选中的城市
        $id.find('ul li ul li').each(function () {
            var _src = $(this).find('div .bbit-tree-node-cb').attr('src');
            _src = _src.substr(_src.lastIndexOf("/") + 1);
            if (_src == 'checkbox_2.png') {
                $(this).find('ul li').each(function () {
                    _src = $(this).find('div .bbit-tree-node-cb').attr('src');
                    _src = _src.substr(_src.lastIndexOf("/") + 1);
                    if (_src == 'checkbox_1.png') {
                        var _text = $(this).find(".bbit-tree-node-text").text();
                        value.push(_text);
                    }
                });
            }
    ...
    $.fn.setNodeClick = function (data) {
        var $id = $(this);
        var id = $id.attr('id').trim();
        var object = $id.find(('#' + id + '_' + data.replace(/-/g, "_")));
        if (object.length != 0) {
    ...
    $.fn.getNode = function (ItemOrItemId) {
        if (this[0].t) {
            return this[0].t.getNode(ItemOrItemId);
        }
    };
