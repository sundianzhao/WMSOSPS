(function ($) {
    $.nfinetab = {
        ...
        addTabs: function (e) {
            ...
            var dataUrl = $(e).attr('href');
            var menuName = $.trim($(e).text());
            var flag = true;
            if (dataUrl == undefined || $.trim(dataUrl).length == 0) {
                return false;
            }
            var $menuTab = window.parent.document.getElementsByClassName('menuTab');
            $($menuTab).each(function () {
                if ($(this).data('id') == dataUrl) {
                    if (!$(this).hasClass('active')) {
                        $(this).addClass('active').siblings('.menuTab').removeClass('active');
            ...
            if (flag) {
                ...
                $($menuTab).removeClass('active');
                var str1 = '<iframe class="NFine_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="100%" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
                var $mainContent = window.parent.document.getElementsByClassName('mainContent');
                $($mainContent).find('iframe.NFine_iframe').hide();
                $($mainContent).append(str1);
                $.loading(true);
                ...
                $($menuTab).parents('.menuTabs').find('.page-tabs-content').append(str);
                $.nfinetab.scrollToTab($($menuTab).find('.active'));
            }
            return false;
        },
        ...
        init: function () {
            ...
            //$('.itemRight').on('click', $.nfinetab.addTabs);
